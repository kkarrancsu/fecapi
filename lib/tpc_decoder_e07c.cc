...
{
    return generic_decoder_sptr(new tpc_decoder(row_polys, col_polys, krow, kcol, bval, qval, max_iter, decoder_type));
}
      ...
      d_bval(bval), d_qval(qval), d_max_iter(max_iter), d_decoder_type(decoder_type)
{
    ...
    extrinsic_info.resize(codeword_M, std::vector<float>(codeword_N, 0));
    ...
    output_c_cols.resize(codeword_M, 0);
    
    output_matrix.resize(codeword_M*codeword_N, 0);
    ...
    beta_row.resize(max_states_row, std::vector<float>(input_u_rows.size()+rowEncoder_K, 0));
    ...
    beta_col.resize(max_states_col, std::vector<float>(input_u_cols.size()+colEncoder_K, 0));
	// forward sweep trellis for current time instant only (t=k)
...
void tpc_decoder::siso_decode_row() {
    ...
    for(state=1; state<max_states_row; state++) {
        beta_row[state][LL+rowEncoder_K-1] = -MAXLOG;
    }
    ...
    // TODO: investigate using algorithm.h's fill function, but memset will NOT work
    // because it take's int values and converts them to chars, and we are trying
    // to initialize a float array, I already tried this and wasted time as a result
    ...
    DEBUG_PRINT_F(fp, "LL=%d KK=%d mm_=%d\n", LL, rowEncoder_K, mm_row);
    for (k=(LL+(rowEncoder_K-1)-1); k>=0; k--) {
        ...
        for ( state=0; state< max_states_row; state++ ) {
            // data 0 branch
            delta1 = beta_row[rowNextStates[0][state]][k+1] + metric_c_row[rowOutputs[0][state]];
            // data 1 branch
            delta2 = beta_row[rowNextStates[1][state]][k+1] + metric_c_row[rowOutputs[1][state]] + app_in;
            // update beta_row
            beta_row[state][k] = (*this.*max_star)(delta1, delta2);
            
             DEBUG_PRINT("delta1=%f delta2=%f beta=%f\n", delta1, delta2, beta_row[state][k]);
             DEBUG_PRINT_F(fp, "delta1=%f delta2=%f beta=%f\n", delta1, delta2, beta_row[state][k]);
        }
                
        // normalize beta_row
        for (state=1;state<max_states_row;state++) {
            beta_row[state][k] = beta_row[state][k] - beta_row[0][k];
        }
        beta_row[0][k] = 0;
    ...
    for(k=1; k<LL+rowEncoder_K; k++) {
        ...
        DEBUG_PRINT("app_in=%f\n", app_in);
		DEBUG_PRINT_F(fp,"app_in=%f\n", app_in);
		...
		for ( state=0; state<max_states_row; state++ ) {
            ...
            DEBUG_PRINT("%f %f %f\n", delta1, delta2, alpha_row[rowNextStates[0][state]]);
            DEBUG_PRINT_F(fp, "%f %f %f\n", delta1, delta2, alpha_row[rowNextStates[0][state]]);
            ...
            DEBUG_PRINT("%f %f %f\n", delta1, delta2, alpha_row[rowNextStates[1][state]]);
			DEBUG_PRINT_F(fp, "%f %f %f\n", delta1, delta2, alpha_row[rowNextStates[1][state]]);
        }
		DEBUG_PRINT("\n");
		DEBUG_PRINT_F(fp, "\n");
        ...
        for (state=0;state<max_states_row;state++)  {
            ...
            delta1 = alpha_prime_row[state] + metric_c_row[rowOutputs[0][state]] +
                     beta_row[rowNextStates[0][state]][k];
            // the information bit
            delta2 = den_llr_u;
            ...
            delta1 = alpha_prime_row[state] + metric_c_row[rowOutputs[1][state]] +
                     beta_row[rowNextStates[1][state]][k] + app_in;
            // the information bit
            delta2 = num_llr_u;
        ...
        for (ii=0;ii<rowEncoder_n;ii++) {
            output_c_rows[rowEncoder_n*(k-1)+ii] = num_llr_c_row[ii] - den_llr_c_row[ii];
        }
    }
}
...
void tpc_decoder::siso_decode_col() {
    ...
    for(state=1; state<max_states_col; state++) {
        beta_col[state][LL+colEncoder_K-1] = -MAXLOG;
    }
    ...
    for (k=(LL+(colEncoder_K-1)-1); k>=0; k--) {
        ...
        memcpy(&rec_array_col[0], &input_c_cols[colEncoder_n*k], sizeof(float)*colEncoder_n);
         ...
         DEBUG_PRINT("rec_array -->\n");
         DEBUG_PRINT_FLOAT_ARRAY_AS_FLOAT(&rec_array_col[0], rec_array_col.size());
         DEBUG_PRINT_F(fp, "rec_array -->\n");
         DEBUG_PRINT_FLOAT_ARRAY_AS_FLOAT_F(fp, &rec_array_col[0], rec_array_col.size());
        ...
        for ( state=0; state< max_states_col; state++ ) {
            // data 0 branch
            delta1 = beta_col[colNextStates[0][state]][k+1] + metric_c_col[colOutputs[0][state]];
            // data 1 branch
            delta2 = beta_col[colNextStates[1][state]][k+1] + metric_c_col[colOutputs[1][state]] + app_in;
            // update beta_col
            beta_col[state][k] = (*this.*max_star)(delta1, delta2);

             DEBUG_PRINT("delta1=%f delta2=%f beta=%f\n", delta1, delta2, beta_col[state][k]);
             DEBUG_PRINT_F(fp, "delta1=%f delta2=%f beta=%f\n", delta1, delta2, beta_col[state][k]);
        }

        // normalize beta_col
        for (state=1;state<max_states_col;state++) {
            beta_col[state][k] = beta_col[state][k] - beta_col[0][k];
        }
        beta_col[0][k] = 0;
    ...
    for(k=1; k<LL+colEncoder_K; k++) {
        ...
        for ( state=0; state<max_states_col; state++ ) {
            ...
            DEBUG_PRINT("%f %f %f\n", delta1, delta2, alpha_col[colNextStates[0][state]]);
			DEBUG_PRINT_F(fp, "%f %f %f\n", delta1, delta2, alpha_col[colNextStates[0][state]]);
            ...
            DEBUG_PRINT("%f %f %f\n", delta1, delta2, alpha_col[colNextStates[1][state]]);
			DEBUG_PRINT_F(fp, "%f %f %f\n", delta1, delta2, alpha_col[colNextStates[1][state]]);
        }
		DEBUG_PRINT("\n");
		DEBUG_PRINT_F(fp, "\n");
        ...
        for (state=0;state<max_states_col;state++)  {
            // data 0 branch (departing)
            delta1 = alpha_prime_col[state] + metric_c_col[colOutputs[0][state]] +
                     beta_col[colNextStates[0][state]][k];
            // the information bit
            delta2 = den_llr_u;
            den_llr_u = (*this.*max_star)(delta1, delta2);
            ...
            delta1 = alpha_prime_col[state] + metric_c_col[colOutputs[1][state]] +
                     beta_col[colNextStates[1][state]][k] + app_in;
            // the information bit
            delta2 = num_llr_u;
            num_llr_u = (*this.*max_star)(delta1, delta2);
            mask = 1<<(colEncoder_n-1);
        ...
        // assign coded outputs
        for (ii=0;ii<colEncoder_n;ii++) {
            output_c_cols[colEncoder_n*(k-1)+ii] = num_llr_c_col[ii] - den_llr_c_col[ii];
        }
    }
}
...
void tpc_decoder::generic_work(void *inBuffer, void *outBuffer) {
    ...
    std::vector<unsigned char> outTmp(d_kcol*d_krow, 0);
    
    int m, n, ii;
    int iter;
    
    // TODO: probably a better way to do this than memcpy?
    // pad the input w/ B zeros
    memcpy(&inputWithPad[d_bval], inPtr, sizeof(float)*inputSize);
    
    DEBUG_PRINT("Input with Pad -->\n");
    DEBUG_PRINT_FLOAT_ARRAY_AS_FLOAT(&inputWithPad[0], inputSizeWithPad);
    DEBUG_PRINT_F(fp, "Input with Pad -->\n");
    DEBUG_PRINT_FLOAT_ARRAY_AS_FLOAT_F(fp, &inputWithPad[0], inputSizeWithPad);
    
    // make a copy of the input into Z & channel_llr
    for(ii=0; ii<codeword_M; ii++) {
        memcpy(&channel_llr[ii][0], &inputWithPad[ii*codeword_N], sizeof(float)*codeword_N);
        memcpy(&Z[ii][0], &inputWithPad[ii*codeword_N], sizeof(float)*codeword_N);
    }
    
    // clear out extrinsic-info between blocks
    for(ii=0; ii<codeword_M; ii++) {
    	memset(&extrinsic_info[ii][0], 0, sizeof(float)*codeword_N);
    	// TODO: THIS IS DANGEROUS CODE.  Since we are initializing the extrinsic_info to
    	// 0's, this is OK but if we had to initialize to a different value MEMSET will NOT work!!
    }
    ...
    for(iter=0; iter<d_max_iter; iter++) {
        ...
        for(m=0; m<codeword_M; m++) {
            // stage the data
            for(ii=0; ii<codeword_N; ii++) {
                input_c_rows[ii] = channel_llr[m][ii] + extrinsic_info[m][ii];
            }
            ...
            // for more iterations
            for(ii=0; ii<codeword_N; ii++) {
                Z[m][ii] = output_c_rows[ii] - extrinsic_info[m][ii];
            }
        }
        ...
        for(n=0; n<codeword_N; n++) {
            ...
            DEBUG_PRINT_FLOAT_ARRAY_AS_FLOAT(&output_c_cols[0], output_c_cols.size());
            DEBUG_PRINT_F(fp, "output_c_cols -->\n");
            DEBUG_PRINT_FLOAT_ARRAY_AS_FLOAT_F(fp, &output_c_cols[0], output_c_cols.size());
            ...
            for(ii=0; ii<codeword_M; ii++) {
                extrinsic_info[ii][n] = output_c_cols[ii] - input_c_cols[ii];
                
                DEBUG_PRINT("%f ", extrinsic_info[ii][n]);
                DEBUG_PRINT_F(fp, "%f ", extrinsic_info[ii][n]);
            }
            DEBUG_PRINT("\n");
            DEBUG_PRINT_F(fp, "\n");
            
            // store this output col into a matrix
            if(iter==(d_max_iter-1)) {      // we only need to store the output on the last iteration
                memcpy(&output_matrix[n*codeword_M], &output_c_cols[0], sizeof(float)*codeword_M);
            }
        }
    }
    ...
    for(m=0; m<d_kcol; m++) {
        for(n=0; n<d_krow; n++) {
            ...
            if(output_matrix[n*codeword_M+m]>0) {
                outTmp[ii++] = (unsigned char) 1;
            }
            else {
                outTmp[ii++] = (unsigned char) 0;
            }
        }
    }
        
    // memcpy the temp otuput (which contains the extra B+Q zeros) into output, which doesn't
    // TODO: make the above for-loop and this below efficient, I think we can do this in one
    // operation instead of the combination of for-loop
    memcpy(out, &outTmp[d_bval+d_qval], sizeof(unsigned char)*get_output_size());
