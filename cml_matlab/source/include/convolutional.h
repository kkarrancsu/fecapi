                        ...
                        int nn )
{
    ...
    for(state=0;state<states;state++) {
        ...
        /*mexPrintf("output_p[%d]=%d trans_p[%d]=%d\n", state, output_p[state], state, trans_p[state]);*/
    }
    return;
}
         ...
         int        nn )
{
  ...
  /*mexPrintf("Bit Encoding -->\n");*/
  for (i=0;i<LL;i++) {
      /*mexPrintf("input[%d]=%u state=%d\n", i, input[i], state);*/
      if (input[i]) {
          /* Input is a one */
  ...
  /*mexPrintf("Tail Encoding -->\n");*/
  if (tail[0] >= 0) {
      for (i=LL;i<LL+KK-1;i++) {
          /*mexPrintf("tail[%d]=%u state=%d\n", state, tail[state], state);*/
          if (tail[state]) {
              /* Input is a one */
              outsym = out1[state];  /* The output symbol */
