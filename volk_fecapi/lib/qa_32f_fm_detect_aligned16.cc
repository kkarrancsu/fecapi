...
#include <volk_fecapi/volk_fecapi_32f_fm_detect_aligned16.h>
#include <cstdlib>
#include <ctime>
...
void qa_32f_fm_detect_aligned16::t1() {
  ...
  for(int count = 0; count < ITERS; ++count) {
    volk_fecapi_32f_fm_detect_aligned16_manual(output0, input0, 1.0, &save, vlen, "generic");
  }
  end = clock();
  ...
  save = 0.1;
  for(int count = 0; count < ITERS; ++count) {
    volk_fecapi_32f_fm_detect_aligned16_manual(output01, input0, 1.0, &save, vlen, "sse");
  }
  end = clock();
  total = (double)(end-start)/(double)CLOCKS_PER_SEC;
  printf("sse_time: %f\n", total);
