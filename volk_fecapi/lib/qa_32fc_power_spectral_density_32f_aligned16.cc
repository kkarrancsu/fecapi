...
#include <volk_fecapi/volk_fecapi_32fc_power_spectral_density_32f_aligned16.h>
#include <cstdlib>
#include <ctime>
...
void qa_32fc_power_spectral_density_32f_aligned16::t1() {
  ...
  for(int count = 0; count < ITERS; ++count) {
    volk_fecapi_32fc_power_spectral_density_32f_aligned16_manual(output_generic, input0, scalar, rbw, vlen, "generic");
  }
  end = clock();
  ...
  start = clock();
  for(int count = 0; count < ITERS; ++count) {
    volk_fecapi_32fc_power_spectral_density_32f_aligned16_manual(output_sse3, input0, scalar, rbw, vlen, "sse3");
  }
  end = clock();
  total = (double)(end-start)/(double)CLOCKS_PER_SEC;
  printf("sse3_time: %f\n", total);
