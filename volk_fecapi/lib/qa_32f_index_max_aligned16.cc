#include <volk_fecapi/volk_fecapi_runtime.h>
...
void qa_32f_index_max_aligned16::t1(){
  ...
  for(int k = 0; k < NUM_ITERS; ++k) {
    volk_fecapi_32f_index_max_aligned16_manual(target_generic, src0, vlen, "generic");
  }
  end = clock();
  ...
  start = clock();
  for(int k = 0; k < NUM_ITERS; ++k) {
    volk_fecapi_32f_index_max_aligned16_manual(target_sse, src0, vlen, "sse2");
  }
  ...
  start = clock();
  for(int k = 0; k < NUM_ITERS; ++k) {
    get_volk_fecapi_runtime()->volk_fecapi_32f_index_max_aligned16(target_sse4_1, src0, vlen);
  }
