...
7, 5, 2, 0, 6, 4, 3, 1, 6, 4, 3, 1, 7, 5, 2, 0, 1, 3, 4, 6, 0, 2, 5, 7, 0, 2, 5, 7, 1, 3, 4, 6 };
  ...
  for(int i = 0; i < num_iters; ++i) {
    volk_fecapi_16s_permute_and_scalar_add_aligned16_manual(target, src0, permute_indexes, cntl0, cntl1, cntl2, cntl3, scalars, num_bytes, "sse2");
  }
  end = clock();
  ...
  start = clock();
  for(int i = 0; i < num_iters; ++i) {
    volk_fecapi_16s_branch_4_state_8_aligned16_manual(target2, src0, permuters, cntl2, cntl3, scalars, "ssse3");
  }
  end = clock();
  ...
  start = clock();
  for(int i = 0; i < num_iters; ++i) {
    volk_fecapi_16s_branch_4_state_8_aligned16_manual(target3, src0, permuters, cntl2, cntl3, scalars, "generic");
  }
  end = clock();
