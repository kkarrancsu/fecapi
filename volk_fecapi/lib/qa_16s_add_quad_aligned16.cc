...
#include <volk_fecapi/volk_fecapi_16s_add_quad_aligned16.h>
#include <cstdlib>
#include <ctime>
//test for sse2
...
void qa_16s_add_quad_aligned16::t1() {
  ...
  for(int count = 0; count < ITERS; ++count) {
    volk_fecapi_16s_add_quad_aligned16_manual(output0, output1, output2, output3, input0, input1, input2, input3, input4, vlen << 1 , "generic");
  }
  end = clock();
  ...
  start = clock();
  for(int count = 0; count < ITERS; ++count) {
    volk_fecapi_16s_add_quad_aligned16_manual(output01, output11, output21, output31, input0, input1, input2, input3, input4, vlen << 1 , "sse2");
  }
  end = clock();
  total = (double)(end-start)/(double)CLOCKS_PER_SEC;
  printf("sse2_time: %f\n", total);
